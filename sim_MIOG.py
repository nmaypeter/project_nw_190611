...
from SeedSelection_MIOG import *
...
ssmiog = SeedSelectionMIOG(graph_dict, seed_cost_dict, product_list)

miog_dict = ssmiog.generateMIOG()
celf_heap = [(round(sum(item[0] for item in miog_dict[i]) * product_list[k][0] / (seed_cost_dict[k][i] if r_flag else 1.0), 4), k, i, 0) for k in range(num_product) for i in miog_dict]
heap.heapify_max(celf_heap)

now_budget, now_profit = 0.0, 0.0
seed_set = [set() for _ in range(num_product)]
seed_miog_seq = [[] for _ in range(num_product)]
...
while now_budget < total_budget and celf_heap:
    ...
    if mep_flag == seed_set_length:
        # print('new seed')
        print(mep_item)
        updateSeedMIOGDict(seed_miog_seq, mep_k_prod, mep_i_node, seed_set, miog_dict[mep_i_node])
        ...
        seed_exp_miog_seq = copy.deepcopy(seed_miog_seq)
        updateSeedMIOGDict(seed_exp_miog_seq, mep_k_prod, mep_i_node, seed_set, miog_dict[mep_i_node])
        expected_inf = calculateMIOGExpectedInf(seed_exp_miog_seq)
