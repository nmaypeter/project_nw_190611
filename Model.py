...
class Model:
    ...
    def model_mioaepw(self, r_flag):
        ...
        for sample_count in range(self.sample_number):
            ...
            celf_heap = [(safe_div(sum(mioa_dict[k][i][j][0] for j in mioa_dict[k][i]) * product_list[k][0], seed_cost_dict[k][i] if r_flag else 1.0), k, i, 0)
            ...
            while temp_sequence:
                ...
                total_budget = safe_div(total_cost, 2 ** b_iter)
    ...
    def model_mioa(self, r_flag):
        ...
        for sample_count in range(self.sample_number):
            ...
            wd_seq = [self.wallet_distribution_type] if self.wallet_distribution_type else self.wd_seq
            mioa_dict = ssmioa_model.generateMIOA()
            celf_heap = [(safe_div(sum(mioa_dict[i][j][0] for j in mioa_dict[i]) * product_list[k][0] * product_weight_list[k], seed_cost_dict[k][i] if r_flag else 1.0), k, i, 0)
            ...
            while temp_sequence:
                ...
                while now_budget < total_budget and celf_heap:
                    ...
                    if mep_flag == seed_set_length:
                        ...
                        seed_set_t = seed_set[mep_k_prod].copy()
                        seed_set_t.add(mep_i_node)
                        dag_k_dict = ssmioa_model.generateDAG2(seed_set_t, {i: mioa_dict[i] for i in seed_set_t})
                        seed_exp_dag_dict[mep_k_prod] = ssmioa_model.generateSeedDAGDict(dag_k_dict, seed_set_t)
                        expected_inf = calculateExpectedInf(seed_exp_dag_dict)
                        ep_t = sum(expected_inf[k] * product_list[k][0] * product_weight_list[k] for k in range(num_product))
                        mg_t = round(ep_t - now_profit, 4)
    ...
    def model_pmis(self):
        ...
        for sample_count in range(self.sample_number):
            ...
            for bi in self.budget_iteration:
                bi_index = self.budget_iteration.index(bi)
                total_budget = safe_div(total_cost, 2 ** bi)
