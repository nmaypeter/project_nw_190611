        ...
        for sample_count in range(self.sample_number):
            ...
            while temp_sequence:
                ...
                while now_budget < total_budget and celf_heap:
                    ...
                    if mep_flag == seed_set_length:
                        updateSeedMIOADict(seed_mioa_dict, mep_k_prod, mep_i_node, seed_set, mioa_dict[mep_i_node])
                        seed_set[mep_k_prod].add(mep_i_node)
                        now_budget = round(now_budget + sc, 4)
                        now_profit = round(now_profit + mep_mg * (sc if r_flag else 1.0), 4)
                        seed_data.append(str(round(time.time() - ss_start_time + ss_acc_time, 4)) + '\t' + str(mep_k_prod) + '\t' + str(mep_i_node) + '\t' +
                        ...
                        seed_exp_mioa_dict = copy.deepcopy(seed_mioa_dict)
                        updateSeedMIOADict(seed_exp_mioa_dict, mep_k_prod, mep_i_node, seed_set, mioa_dict[mep_i_node])
