...
from SeedSelection_MDAGEPW import *
...
ssmdagepw = SeedSelectionMDAGEPW(graph_dict, seed_cost_dict, product_list, product_weight_list)

mioa_dict = ssmdagepw.generateMIOA()
mioa_dict = ssmdagepw.updateMIOAEPW(mioa_dict)
celf_heap = [(round(sum(mioa_dict[k][i][j][0] for j in mioa_dict[k][i]) * product_list[k][0] / (seed_cost_dict[k][i] if r_flag else 1.0), 4), k, i, 0) for k in range(num_product) for i in mioa_dict[k]]
